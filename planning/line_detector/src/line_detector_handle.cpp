...
void LineDetectorHandle::loadParameters() {
  ...
  if (!nodeHandle_.param<std::string>("perception_cone_topic_name",
                                      lidar_cluster_topic_name_,
                                      "/local_map_acceleration")) {
    ROS_WARN_STREAM("Did not load perception_cone_topic_name. Standard value is: " << lidar_cluster_topic_name_);
  }
...
void LineDetectorHandle::lidarClusterCallback(const fsd_common_msgs::Map &msg) {
  //将局部地图内的锥桶位置信息转换为 基于世界坐标系下的位置
